...
void init_matrix(double* m, int rows, int columns) {
    // schedule: guided
    // Entrega o primeiro bloco bem grande para as primeiras threads:
    // e Entrega columns j's para cada thread
...
void mult_matrix(double* out, double* left, double *right,
    ...
    int i, j, k;
    // Nao precisa criar dois pragmas, o i, por padrao eh private.
    // Precisa privatizar j e k porque o pragma soh garante o i
    // Logo j e k estao sendo mexid
